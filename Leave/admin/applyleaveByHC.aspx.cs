...
public partial class leave_applyleaveByHC : System.Web.UI.Page
{
    ...
    private void fillControls()
    {
        ...
        DataTable dt_result = balObj.getCurrentEmployeeList(Convert.ToBoolean(0));
        if (dt_result.Rows.Count > 0)
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "nouser", "alert('Some technical issue occur on page, please contact to administrator');window.open('leaveadmin.aspx','_self')", true);
            }
        }
        else
        {
    ...
    protected void bindemployee_detail()
    {
        ...
        sqlparm.Value = ddl_employee.SelectedItem.ToString();
        DataSet ds_result = DBTask.ExecuteDataset(ConfigurationManager.ConnectionStrings["RossellConnectionString"].ConnectionString.ToString(), CommandType.StoredProcedure, "sp_leave_viewempdetail_forOD", sqlparm);

        if (ds_result.Tables[0].Rows.Count > 0)
        {
            //lbl_emp_name.Text = ds.Tables[0].Rows[0]["name"].ToString();
            lbl_emp_code.Text = ds_result.Tables[0].Rows[0]["empcode"].ToString();
            lbl_gender.Text = ds_result.Tables[0].Rows[0]["emp_gender"].ToString();
            //HiddenField_gender.Value = ds.Tables[0].Rows[0]["emp_gender"].ToString();
            lbl_emp_status.Text = ds_result.Tables[0].Rows[0]["status"].ToString();
            lbl_department.Text = ds_result.Tables[0].Rows[0]["CategoryName"].ToString();
            //lbl_branch.Text = ds_result.Tables[0].Rows[0]["BussinessUnitName"].ToString();
            // lbl_branch.Text = ds.Tables[0].Rows[0]["branch_name"].ToString();
            lbl_designation.Text = ds_result.Tables[0].Rows[0]["designationname"].ToString();
            if (!string.IsNullOrEmpty(ds_result.Tables[0].Rows[0]["emp_doj"].ToString()))
            {
                lbl_doj.Text = Convert.ToDateTime(ds_result.Tables[0].Rows[0]["emp_doj"]).ToString("dd-MMM-yyyy");
            }
            ViewState["UserExist"] = true;
            BindLeaveType();
            if (ds_result.Tables.Count > 1)
            {
                grdMyBalance.DataSource = ds_result.Tables[1];
    ...
    protected void BindLeaveType()
    {
        ...
        if (!string.IsNullOrEmpty(lbl_emp_code.Text.Trim()))
        {
            sqlparm[0].Value = lbl_emp_code.Text.Trim();
            ...
            {
                if (Convert.ToDecimal(_dr["balance"]) <= 0)
                {
                    rows.Add(_dr);
                }
            }
    ...
    public void _BindddlDirector()
    {
        ...
        objEmpJobDetailENT.EmployeeCode = ddl_employee.SelectedValue.ToString();
    ...
    protected void dd_typeleave_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        sqlparm[0] = new SqlParameter("@empcode", SqlDbType.VarChar, 100);
        sqlparm[0].Value = ddl_employee.SelectedValue.ToString();
    ...
    {
        SqlParameter[] sqlparm = new SqlParameter[22];
        ...
        sqlparm[19] = new SqlParameter("@short", SqlDbType.Bit, 1);
        sqlparm[20] = new SqlParameter("@director", SqlDbType.VarChar, 100);
        sqlparm[21] = new SqlParameter("@ResPerson", SqlDbType.VarChar, 100);

        if (divfull.Visible == true)
        {
            sqlparm[2].Value = 1;
            sqlparm[3].Value = Convert.ToDateTime(txt_sdate.Text);
            sqlparm[4].Value = Convert.ToDateTime(txt_edate.Text);
            sqlparm[5].Value = System.Data.SqlTypes.SqlDateTime.Null;
            sqlparm[7].Value = System.Data.SqlTypes.SqlByte.Null;
            sqlparm[19].Value = System.Data.SqlTypes.SqlByte.Null;
            if (divDirector.Visible == true)
            {
                sqlparm[20].Value = ddlDirector.SelectedValue;
                sqlparm[21].Value = txt_employee.Text;
            }
            else
            {
                sqlparm[20].Value = System.Data.SqlTypes.SqlString.Null;
                sqlparm[21].Value = System.Data.SqlTypes.SqlString.Null;
            }
        }
        ...
        else
        {
            ...
            sqlparm[7].Value = System.Data.SqlTypes.SqlByte.Null;
            sqlparm[19].Value = opt_inFirst.Checked;
            sqlparm[20].Value = System.Data.SqlTypes.SqlString.Null;
            sqlparm[21].Value = System.Data.SqlTypes.SqlString.Null;

        }
        int result = Convert.ToInt32(DBTask.ExecuteScalar(ConfigurationManager.ConnectionStrings["RossellConnectionString"].ConnectionString.ToString(), CommandType.StoredProcedure, "sp_leave_applyleave", sqlparm));
        string query = "update tbl_leave_employee_leave_master set used_days+='" + txt_nod.Text.Trim() + "' where empcode='" + ddl_employee.SelectedValue.Trim() + "' and leaveid='" + dd_typeleave.SelectedValue + "'";
        DBTask.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["RossellConnectionString"].ConnectionString.ToString(), CommandType.Text, query, null);
        return result;
    }
    ...
    protected void btn_submit_Click(object sender, EventArgs e)
    {
        if (dd_typeleave.SelectedItem.Text != "Comp-Off")
        {
            ...
            try
            {
                ...
                int leaveid = insertapplyleave(6);
                insertadjustment(leaveid);
                string empcode = ddl_employee.SelectedValue.ToString();
                ...
                ScriptManager.RegisterStartupScript(this, GetType(), "leaveapplied", "alert('Leave have been applied and approved');", true);
                clearAll();
                clearDates();
            }
            catch (Exception ex)
        ...
        {
            InsertCompoff();
            ScriptManager.RegisterStartupScript(this, GetType(), "leaveapplied", "alert('Compensatory leave have been applied and approved');", true);
        }
    ...
    {
        Page.Validate("a");
        int CompOffid = insertapplycompoff(6);
        string empcode = ddl_employee.SelectedValue.ToString();
        if (divfull.Visible == true)
        {
    ...
    {
        SqlParameter[] sqlparm = new SqlParameter[16];
        ...
        if (divfull.Visible == true)
        {
            sqlparm[1].Value = Convert.ToDateTime(txt_sdate.Text.Trim().ToString());
            sqlparm[2].Value = Convert.ToDateTime(txt_edate.Text.Trim().ToString());
        }
        else
        {
            sqlparm[1].Value = Convert.ToDateTime(txt_select.Text.Trim().ToString());
        ...
        sqlparm[14] = new SqlParameter("@fileType", SqlDbType.VarChar, 100);
        sqlparm[14].Value = "";

        sqlparm[15] = new SqlParameter("@halfMode", SqlDbType.Bit);
        sqlparm[15].Value = Convert.ToBoolean(opt_first.Checked);
        ...
        try
        {
            ...
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    string subjectForSelf = "Leave Notification –Your leave has been applied and approved for " + dt_result.Rows[0]["no_of_days"].ToString() + " Day(s) of Vacation";
                    ...
                    string table = "<br/><br/><table><colgroup><col style='width:35%;background: #edf5ff; padding: 4px 0 4px 5px; border: 1px solid #c9dffb; border-right:none; font: bold 13px verdana, Helvetica, sans-serif; color: #013366;' />" +
                       ...
                       "<tr><td>Leave Type</td><td>" + dt_result.Rows[0]["leavetype"].ToString() + "</td></tr><tr><td>Reason</td><td>" + dt_result.Rows[0]["reason"].ToString() + "</td></tr><tr><td>Remark</td><td>" + dt_result.Rows[0]["comments"].ToString() + "</td></tr></table>" +
               ...
               "<h4>Hi " + dt_result.Rows[0]["EmpName"].ToString() + "</h4><br /> Your leave have applied and approved by " + Session["EmpName"].ToString() + ". The leave details are:     <br /> ");
                    ...
                    //       builder.Clear();
                    //       builder.Append("<div runat='server' id='dv_head' style='background-color: 	#E8E8E8 ;width:500px;height:300px;font:  bold 12px verdana, Helvetica, sans-serif;'>" +
                    //"<h4>Hi " + dt_result.Rows[0]["ApproverName"].ToString() + "</h4><br /> " + dt_result.Rows[0]["EmpName"].ToString() + "  has applied for leave<br/>login to HCMS portal for Approving/Rejecting the  leave application. The leave details are:     <br /> ");
                    //       builder.Append(table);
                    ...
                    //       builder.Clear();
                    //       builder.Append("<div runat='server' id='dv_head' style='background-color: 	#E8E8E8 ;width:500px;height:300px;font:  bold 12px verdana, Helvetica, sans-serif;'>" +
                    //"<h4>Hi " + dt_result.Rows[1]["ApproverName"].ToString() + "</h4><br /> " + dt_result.Rows[0]["EmpName"].ToString() + "  has applied for leave. The leave details are:     <br /> ");
                    //       builder.Append(table);
        ...
        try
        {
            ...
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    #region MailtoSelf
                    builder.Append("<div runat='server' id='dv_head' style='background-color: 	#E8E8E8 ;width:500px;height:300px;font: bold 12px verdana, Helvetica, sans-serif;'>" +
               "<h4>Hi " + dt_result.Rows[0]["EmpName"].ToString() + "</h4><br /> You leave have been applied for half day by " + Session["EmpName"].ToString() + ". The leave details are:     <br /> ");
                    builder.Append(table);
                    mail.sendMailWithFormat(dt_result.Rows[0]["EmpMailID"].ToString(), subjectForSelf, builder.ToString(), null, null);
                    #endregion
                    //       #region MailtoRM
                    //       builder.Clear();
                    //       builder.Append("<div runat='server' id='dv_head' style='background-color: 	#E8E8E8 ;width:500px;height:300px;font:  bold 12px verdana, Helvetica, sans-serif;'>" +
                    //"<h4>Hi " + dt_result.Rows[0]["ApproverName"].ToString() + "</h4><br /> " + dt_result.Rows[0]["EmpName"].ToString() + "  has applied for half day leave.<br/>login to HCMS portal for Approving/Rejecting the  leave application. The leave details are:     <br /> ");
                    //       builder.Append(table);
                    //       //   mail.sendMailWithFormat(dt_result.Rows[0]["EmailID"].ToString(), subjectForRM, builder.ToString(), null, null);  
                    //       #endregion
                    //       #region MailtoHC
        ...
        try
        {
            ...
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    string subjectForSelf = "Leave Notification –Your Compensatory leave have applied and approved for " + dt_result.Rows[0]["no_of_days"].ToString() + " Day(s) ";
                    ...
                    string table = "<br/><br/><table><colgroup><col style='width:35%;background: #edf5ff; padding: 4px 0 4px 5px; border: 1px solid #c9dffb; border-right:none; font: bold 13px verdana, Helvetica, sans-serif; color: #013366;' />" +
                        "<col style='width:65%;background: #f8fbff; padding: 4px 0 4px 5px; border: 1px solid #c9dffb; border-right:none; font: bold 11px verdana, Helvetica, sans-serif; color: #013366;' />" +
                       " </colgroup><tr><td>From Date</td><td>" + Convert.ToDateTime(dt_result.Rows[0]["fromdate"].ToString()).ToString("dd-MMM-yyyy") + "</td></tr><tr><td>To Date</td><td>" + Convert.ToDateTime(dt_result.Rows[0]["todate"].ToString()).ToString("dd-MMM-yyyy") + "</td></tr>" +
                       "<tr><td>Leave Type</td><td>Compensatory leave</td></tr><tr><td>Reason</td><td>" + dt_result.Rows[0]["reason"].ToString() + "</td></tr><tr><td>Remark</td><td>" + dt_result.Rows[0]["comment"].ToString() + "</td></tr></table>" +
                      "Thanks,<br />Acuminous Software<br /><br /></div>";
                    ...
                    #region MailtoSelf
                    builder.Append("<div runat='server' id='dv_head' style='background-color: 	#E8E8E8 ;width:500px;height:300px;font:  bold 12px verdana, Helvetica, sans-serif;'>" +
               "<h4>Hi " + dt_result.Rows[0]["EmpName"].ToString() + "</h4><br /> You Compensatory leave have applied and appproved by " + Session["EmpName"].ToString() + ". The leave details are:     <br /> ");
                    builder.Append(table);
                    mail.sendMailWithFormat(dt_result.Rows[0]["EmpMailID"].ToString(), subjectForSelf, builder.ToString(), null, null);
                    #endregion
                    #region MailtoRM
        ...
        try
        {
            ...
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    #region MailtoSelf
                    builder.Append("<div runat='server' id='dv_head' style='background-color: 	#E8E8E8 ;width:500px;height:300px;font: bold 12px verdana, Helvetica, sans-serif;'>" +
               "<h4>Hi " + dt_result.Rows[0]["EmpName"].ToString() + "</h4><br /> You Compensatory leave have applied and approved for half day leave. The leave details are:     <br /> ");
                    builder.Append(table);
                    mail.sendMailWithFormat(dt_result.Rows[0]["EmpMailID"].ToString(), subjectForSelf, builder.ToString(), null, null);
                    #endregion
                    //       #region MailtoRM
    ...
    protected Boolean validateapplyleave()
    {
        ...
        sqlparm[2] = new SqlParameter("@startdate", SqlDbType.DateTime, 10);
        sqlparm[3] = new SqlParameter("@enddate", SqlDbType.DateTime, 10);
    ...
    protected Boolean validate_applydateCompOff()
    {
        ...
        sqlparam[0].Value = ddl_employee.SelectedValue.ToString();
    ...
    protected void validateapplycompoff()
    {
        ...
        if (divfull.Visible == true)
        {
            sqlparm[2].Value = Utility.DateTimeForat(txt_sdate.Text.Trim().ToString());
            sqlparm[3].Value = Utility.DateTimeForat(txt_edate.Text.Trim().ToString());
        }
        else
        {
            sqlparm[2].Value = Utility.DateTimeForat(txt_select.Text.Trim().ToString());
            sqlparm[3].Value = Utility.DateTimeForat(txt_select.Text.Trim().ToString());
        }
    ...
    protected Boolean valdiate_leave(DataSet ds)
    {
        ...
        if (divfull.Visible == true)
            ...
            sdate = Utility.DateTimeForat(txt_select.Text);
        else
            sdate = Utility.DateTimeForat(txt_selectSh.Text);
    ...
    protected Boolean validate_dutyroster()
    {
        ...
        sqlpar[0].Value = ddl_employee.SelectedValue.ToString();
        ...
        if (divfull.Visible == true)
        {
            sqlpar[1].Value = Convert.ToDateTime(txt_sdate.Text);
            sqlpar[2].Value = Convert.ToDateTime(txt_edate.Text);
        }
        else if (divhalf.Visible == true)
        {
            sqlpar[1].Value = Convert.ToDateTime(txt_select.Text);
            sqlpar[2].Value = Convert.ToDateTime(txt_select.Text);
        }
        else
        {
            sqlpar[1].Value = Convert.ToDateTime(txt_selectSh.Text);
            sqlpar[2].Value = Convert.ToDateTime(txt_selectSh.Text);
        }
