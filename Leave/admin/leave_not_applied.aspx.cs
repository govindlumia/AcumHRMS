...
using Utilities;
public partial class leave_admin_leave_not_applied : System.Web.UI.Page
{
    string sqlstr;
    ...
    protected void bindempdetail()
    {
        ...
        ds = DBTask.ExecuteDataset(ConfigurationManager.ConnectionStrings["RossellConnectionString"].ConnectionString.ToString(), CommandType.StoredProcedure, "[sp_payroll_fetch_employee_absentee]", sqlparam);
    ...
    protected void sendmail()
    {
        try
        {
            ...
            foreach (GridViewRow GridView in empgrid.Rows)
            {
                ...
                if (checkg.Checked)
                {
                    string strempcode,date, mailto, mailbody;
                    strempcode = ((Label)empgrid.Rows[counter].FindControl("lblempcodeg")).Text;
                    date = ((Label)empgrid.Rows[counter].FindControl("lbldate")).Text;
                    ...
                    if (ds5.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (mailto.Trim() != "")
                        {
                            ...
                            mail.To = mailto;
                            mail.Subject = "Non Punched Card on Dated : " + date;
                            mail.BodyFormat = MailFormat.Html;
                            mail.Body = "Dear Sir/Madam";

                            mail.Body = mail.Body + "<br />You have not punched your card. Your attendence will be marked as absent for this day. Please apply leave for same day.";
