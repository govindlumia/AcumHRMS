...
public partial class Appraisal_UserControl_UCKRAFormReviewerRating : BasePageUC<KRAFormEntity>
{
    KRAFormBAL _objBAL;
    public Appraisal_UserControl_UCKRAFormReviewerRating()
    {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                Response.Redirect("KRAFormReviewer.aspx");
            }
    ...
    public void BindPromotionCheckList()
    {
        ...
        strPromotionQuery = String.Format(strPromotionQuery, ViewState["KRAFormID"].ToString(), 2);
        ...
        try
        {
            ...
            if (Convert.ToInt64(ViewState["KRAFormID"]) > 0)
            {
                ...
                foreach (GridViewRow row in grdTraining.Rows)
                {
                    ...
                    kraTraining.ApproverCode = "Reviewer";
                ...
                foreach (GridViewRow row in grdPromotion.Rows)
                {
                    ...
                    {
                        var kraPromotion = new AppraisalPromotionEntity() { Comment = txtPromotionComment.Text, KRAFormID = kraForm.Id, PromotionId = Convert.ToInt32(lblPromotion.ToolTip), CreatedBy = SessionEmpCode, LevelId = 2, PostAction = chkPromotion.Checked };
                ...
                if (bal.InsertKRARating(kraForm) > 0)
                {
                    if (!kraForm.IsDraft)
                        SendMail();
                    General.Alert("Appraisal Form updated successfully.", btnClose, "../Admin/AllAppraisalStatus.aspx?User=R");
                }
                else
        ...
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    message.Subject = "Appraisal Notification – Appraisal Submitted for Final Review by " + Session["EmpName"].ToString() + " for" + lblUserName.Text;
                    message.IsBodyHtml = true;
                    message.Body = "Hi " + ds.Tables[0].Rows[0]["EmpName"].ToString() + " <br> " + Session["EmpName"].ToString() + " has submitted the appraisal for " + lblUserName.Text + ".<br>Login to HCMS portal to complete the appraisal process.<br><br>Thanks<br>Acuminous Software<br><br><br><br>This is an automated notification.<br>";
    ...
    {
        string queryDelete = "Delete from KRAFormRatingDetail where KRAFormId=" + kraFormId.ToString() + " and LevelId=2 Delete from KRAFormPromotion where KRAFormId=" + kraFormId.ToString() + " and LevelId=2";
    ...
    private String strComment = "Select Comment from KRAFormRatingDetail Where KRAFormId={0} and KRASettingDetailId={1} and LevelId=2";
    protected void grdDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            Label reviewerRating = (Label)e.Row.FindControl("reviewerRatingLabel");
            if (!String.IsNullOrEmpty(reviewerRating.Text) && reviewerRating.Text != "0")
            {
                ddlRating.Items.FindByText(reviewerRating.Text).Selected = true;
            }
    ...
    private void BindHistoryComments()
    {
        ...
        foreach (GridViewRow grd in grdHistory.Rows)
        {
            ...
            if (status.Text.Contains("Reviewer"))
            {
