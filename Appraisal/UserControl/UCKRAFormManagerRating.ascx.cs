...
public partial class Appraisal_UserControl_UCKRAFormManagerRating : BasePageUC<KRAFormEntity>
{
    KRAFormBAL _objBAL;
    public Appraisal_UserControl_UCKRAFormManagerRating()
    {
    ...
    public void BindPromotionCheckList()
    {
        ...
        //if (ViewState["FromPeer"] != null)
        //{
        strPromotionQuery = String.Format(strPromotionQuery, ViewState["KRAFormID"].ToString(), 1);
        ...
        try
        {
            ...
            if (Convert.ToInt64(ViewState["KRAFormID"]) > 0)
            {
                ...
                foreach (GridViewRow row in grdTraining.Rows)
                {
                    ...
                    kraTraining.ApproverCode = "Manager";
                ...
                foreach (GridViewRow row in grdKRAForm.Rows)
                {
                    ...
                    foreach (GridViewRow rowDetail in grdPK.Rows)
                    {
                        ...
                        CheckBox chk = (CheckBox)rowDetail.FindControl("chkKRADetailId");
                        TextBox txt = (TextBox)rowDetail.FindControl("txtEmpCode");
                        ...
                        formDetail.IsPeerManager = chk.Checked ? true : false;
                        formDetail.PeerManagerCode = formDetail.IsPeerManager ? txt.Text : "";
                ...
                foreach (GridViewRow row in grdPromotion.Rows)
                {
                    ...
                    {
                        var kraPromotion = new AppraisalPromotionEntity() { Comment = txtPromotionComment.Text, KRAFormID = kraForm.Id, PromotionId = Convert.ToInt32(lblPromotion.ToolTip), CreatedBy = SessionEmpCode, LevelId = 1, PostAction = chkPromotion.Checked };
        ...
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    message.Subject = "Appraisal Notification – Appraisal Submitted for Review by " + Session["EmpName"].ToString();
                    message.IsBodyHtml = true;
                    message.Body = "Hi " + ds.Tables[0].Rows[0]["EmpName"].ToString() + " <br> " + Session["EmpName"].ToString() + " has submitted the appraisal for " + lblUserName.Text + "<br>Login to HCMS portal to proceed with appraisal process.<br><br>Thanks<br>Acuminous Software<br><br><br><br>This is an automated notification.<br>";
    ...
    {
        string queryDelete = "Delete from KRAFormRatingDetail where KRAFormId=" + kraFormId.ToString() + " and LevelId=1 Delete from KRAFormPromotion where KRAFormId=" + kraFormId.ToString() + " and LevelId=1";
    ...
    private String strComment = "Select Comment from KRAFormRatingDetail Where KRAFormId={0} and KRASettingDetailId={1} and LevelId=1";
    protected void grdDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            CheckBox chk = (CheckBox)e.Row.FindControl("chkKRADetailId");
            HtmlGenericControl div = (HtmlGenericControl)e.Row.FindControl("divPeerManager");
            HtmlGenericControl divManager = (HtmlGenericControl)e.Row.FindControl("divManager");
            //chk.Attributes.Add("onchange", "return peerManagerCheck(" + div.ClientID + "," + chk.ClientID + "," + divManager.ClientID + ")");

            TextBox txt = (TextBox)e.Row.FindControl("txtEmpCode");
            HtmlAnchor anchor = (HtmlAnchor)e.Row.FindControl("lnkA");
            txt.Attributes.Add("readonly", "readonly");
            anchor.HRef = "JavaScript:newPopup1('../../pickempwithdata.aspx?HR=0&MSS=0&Emp=1&Con=" + txt.ClientID + "')";
            anchorAll.HRef = "JavaScript:newPopup1('KRAComments.aspx?FormId=" + ViewState["KRAFormID"].ToString() + "&SettingId=" + setting.Text + "')";
            var query = String.Format(strKRAComment, ViewState["KRAFormID"].ToString(), setting.Text);
            var count = DBTask.ExecuteDataset(ConfigurationManager.ConnectionStrings["RossellConnectionString"].ConnectionString.ToString(), CommandType.Text, query).Tables[0].Rows[0][0].ToString();
            if (count != "0")
            {
    ...
    protected void chk_CheckedChanged(object sender, EventArgs e)
    {
        ...
        var kraDetailId = chk.ToolTip.ToString();

        foreach (GridViewRow row in grdKRAForm.Rows)
        {
            GridView grd = (GridView)row.FindControl("grdDetails");

            foreach (GridViewRow rowInner in grd.Rows)
            {
                CheckBox chkGrd = (CheckBox)rowInner.FindControl("chkKRADetailId");

                if (chkGrd.ToolTip.ToString() == kraDetailId)
                {
                    HtmlGenericControl div = (HtmlGenericControl)rowInner.FindControl("divPeerManager");
                    HtmlGenericControl divManager = (HtmlGenericControl)rowInner.FindControl("divManager");
    ...
    private String strKRAComment = "Select count(Comment) from KRAFormDetail Where KRAFormId={0} and KRASettingDetailId={1} and isnull(Comment,'')!=''";
    ...
    private void BindHistoryComments()
    {
        ...
        foreach (GridViewRow grd in grdHistory.Rows)
        {
            ...
            if (status.Text.Contains("Manager"))
            {
